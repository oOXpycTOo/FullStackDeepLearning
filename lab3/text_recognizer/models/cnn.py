...
class SqueezeExcitationBlock(nn.Module):
    ...
    def _get_residual_and_identity(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        out = self.residual_block.conv1(x)
        out = self.residual_block.bn1(out)
        out = self.residual_block.relu(out)
        out = self.residual_block.conv2(out)
        out = self.residual_block.bn2(out)
        return out, identity
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        res_weights = res_weights.unsqueeze(-1).unsqueeze(-1)
        res = res_weights * res
        return res + identity
