...
class CTCLitModel(BaseLitModel):  # pylint: disable=too-many-ancestors
    ...
    def training_step(self, batch, batch_idx):  # pylint: disable=unused-argument
        ...
        B, S, _C = logprobs.shape

        logprobs_for_loss = logprobs.permute(1, 0, 2)  # -> (S, B, C)
    ...
    def validation_step(self, batch, batch_idx):  # pylint: disable=unused-argument
        ...
        logprobs_for_decode = logprobs.permute(0, 2, 1)
        logprobs_for_loss = logprobs.permute(1, 0, 2)  # -> (S, B, C)
        ...
        decoded = self.greedy_decode(logprobs_for_decode, max_length=y.shape[1])
    ...
    def test_step(self, batch, batch_idx):  # pylint: disable=unused-argument
        ...
        logprobs_for_decode = logprobs.permute(0, 2, 1)
        decoded = self.greedy_decode(logprobs_for_decode, max_length=y.shape[1])
        all_classes = torch.arange(decoded.max(), device=y.device)
        decoded_probs = (decoded.unsqueeze(-1) == all_classes.reshape(1, 1, -1)).float()
        self.test_acc(decoded_probs, y)
